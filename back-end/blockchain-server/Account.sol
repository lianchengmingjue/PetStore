...
contract Account {
    ...
    function createTable() private {
        ...
        // |  账户id即邮箱(主键)  |    密码    |  用户昵称  |  账户余额   |  管理员权限   |
        // |-------------------- |------------|------------|------------|--------------|
        // |        user_id      |  password  |    name    |  balance   | administrator|
        // |---------------------|------------|------------|------------|--------------|
        //
        // 创建表
        tf.createTable("t_account", "user_id", "password,name,balance,administrator");
    }
    ...
    返回值：
            ...
            -1 账户注册失败
    */
    function regist(string user_id, string password,string name,string balance,string administrator,string emit_description) public returns(int256){
        int256 ret_code = 0;
        Table table = openTable();
    ...
    */
    function update(string user_id, string password,string name,string balance,string administrator,string emit_description) public returns(int256){
        int256 ret_code = 0;
        Table table = openTable();
    ...
    返回值：
            ...
            参数二： 第一个参数为0时有效，账户密码，账户余额等信息
    */
    function select(string user_id) public constant returns(int256, string,string,string,string) {
        // 打开表
        Table table = openTable();
        // 查询
        Entries entries = table.select(user_id, table.newCondition());
        if (0 == uint256(entries.size())) {
            ...
            return (0,entry.getString("password"),entry.getString("name"),entry.getString("balance"),entry.getString("administrator"));
        }
    }
